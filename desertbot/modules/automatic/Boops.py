...
class Boops(BotModule):
    ...
    def respond(self, message: IRCMessage) -> IRCResponse:
        ...
        if message.command == "boop":
            ...
            if match and (datetime.datetime.utcnow() - self.lastTriggered).seconds >= self.cooldown:
                ...
                return IRCResponse(f"Boop! {random.choice(self.storage['boops'])}", message.replyTo)
    ...
    def _addBoop(self, message: IRCMessage) -> IRCResponse:
        ...
        return IRCResponse(f"Added {message.parameterList[1]} to the list of boops!", message.replyTo)
    ...
    def _removeBoop(self, message: IRCMessage) -> IRCResponse:
        if message.parameterList[1] in self.storage['boops']:
            ...
            return IRCResponse(f"Removed {message.parameterList[1]} from the list of boops!", message.replyTo)
        else:
            return IRCResponse(
                f"Couldn't find {message.parameterList[1]} in the list of boops, did you maybe do a typo?",
                message.replyTo)
